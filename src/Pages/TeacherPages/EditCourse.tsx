import Button from "../../Components/ui/Button";
import Input from "../../Components/ui/Input";
import NavBar from "../../Layouts/NavBar";
import { useCustomModal } from "../../hooks/useCustomModal";
import { useForm } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import { editCourseShcema } from "../../schema";
import CustomModal from "../../Components/ui/CustomModal";
import { useAuth } from "../../context/AuthContext";
import { useTeacherEditCourses } from "../../hooks/useTeacher";
import { useParams } from "react-router-dom";
import CircularProgress from "@mui/material/CircularProgress";

interface IFormData {
  coursename: string;
  year: number;
  department: string;
}

const EditCourse = () => {
  const { handleLogout } = useAuth();
  const { mutate: EditCourse, isPending } = useTeacherEditCourses();
  const { isCustomModalOpen, setIsCustomModalOpen } = useCustomModal();
  const {id} = useParams();
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm({
    resolver: yupResolver(editCourseShcema),
  });

  const onSubmit = (data: IFormData) => {
    if (!id) return; // narrows id to string for TS, and guards against missing param
    
    EditCourse({ id, data });
    reset();
  };

  return (
    <div className="page-shell">
      <NavBar
        description=""
        mainMessage="Edit Course"
        onLogout={() => setIsCustomModalOpen(true)}
      />
      <form
        className="mt-12 sm:ml-5 space-y-10"
        onSubmit={handleSubmit(onSubmit)}
      >
        <div className="flex flex-col space-y-0.5">
          <Input {...register("coursename")} label="course name" />
          <p className="text-red-600 text-lg">{errors.coursename?.message}</p>
        </div>
        <div className="flex flex-col space-y-0.5">
          <Input {...register("year")} label="year" />
          <p className="text-red-600 text-lg">{errors.year?.message}</p>
        </div>
        <div className="flex flex-col space-y-0.5">
          <Input
            {...register("department")}
            label="department"
          />
          <p className="text-red-600 text-lg">{errors.department?.message}</p>
        </div>
        <Button
          text={
            isPending ? (
              <CircularProgress size={25} aria-label="Loading…" />
            ) : (
              "update"
            )
          }
        />
      </form>

      {/* LOGOUT MODAL */}
      <CustomModal
        cancelTxt="No"
        confirmTxt="Yes"
        mainTxt="Are you sure you want to logout ?"
        onConfirm={handleLogout}
        open={isCustomModalOpen}
        setIsOpen={setIsCustomModalOpen}
      />
    </div>
  );
};

export default EditCourse;
